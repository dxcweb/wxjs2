...
import QyWxUploadImgToOss from './QyWxUploadImgToOss'



export default class QyWxImagePicker extends Component {
    state = {
    ...
    getImageUrl(value) {
        return location.protocol + value.host + value.url + "?x-oss-process=image/quality,q_99/format,jpg";
    }

    uploadImage(serverId, localId, callback) {
        this.setState({serverId});
        QyWxUploadImgToOss(qy_wx_url, qy_wx_app_id, serverId).then(function (res) {
            if (res.result) {
                callback(res.data);
            } else {
                alert(res.msg);
